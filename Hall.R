...
ps_error = propProdErrors(ps, c(coef(lm(Islopes ~ Bvalues, IDF))[2]), 0.239)

print(paste("Valor hallado para ps: ", signif(ps, 3), " pm ", signif(ps_error, 3)))
...
RHI_error = propProdErrors(RH_I, c(coef(lm(Islopes ~ Bvalues, IDF))[2]), c(0.239))
RH_B = d * coef(lm(Bslopes ~ Ivalues, BDF))[2]
RHB_error = propProdErrors(RH_B, c(coef(lm(Bslopes ~ Ivalues, BDF))[2]), c(0.166))

RH_mean = (RH_I + RH_B)/2
RH_mean_error = propEscErrors(0.5, propSumErrors(c(RHI_error, RHB_error)))
...
print(paste("Estimación R_H dado I variable: ", signif(RH_I, 3), " pm ", signif(RHI_error, 3)))
print(paste("Estimación R_H dado B variable: ", signif(RH_B, 3), " pm ", signif(RHB_error, 3)))
print(paste("Estimación R_H promedio de I B: ", signif(RH_mean, 3), " pm ", signif(RH_mean_error, 3)))
...
legend("topleft", legend=c(
    unlist(lapply(Ivalues, function(val) paste("I = ", val, " [mA]")))
  ),
  col=c(6:1), cex = 1.35, lwd = 2)



# Gráficas de U_H vs I
par(mar=c(5,6,4,1)+.1)
plot(Idata[[6]], col = 6,
     xlab = "I [mA]",
     ylab = "U_H [V]",
...
legend("topleft", legend=c(
  unlist(lapply(Bvalues, function(val) paste("B = ", val, " [mT]")))
  ),
  col=c(6:1), cex = 1.35, lwd = 2)
...
plot(Islopes, Bvalues, col = 1,
     ...
     pch = 1,
     lwd = 2)
abline(lm(Bvalues ~ Islopes, IDF), lwd = 2)


# Gráfica I vs A_B
par(mar=c(5,6,4,1)+.1)
plot(Bslopes, Ivalues, col = 1,
     xlab = "I [mA]",
     ylab = "A_B [V/T]",
     cex = 1.5,
     cex.lab = 2.5,
     ...
     pch = 1,
     lwd = 2)
abline(lm(Ivalues ~ Bslopes, IDF), lwd = 2)


print("")
print("")

print(paste("Pendiente A_I vs B: ", coef(lm(Islopes ~ Bvalues, IDF))[2], " pm 0.239"))
print(paste("Pendiente A_B vs I: ", coef(lm(Bslopes ~ Ivalues, IDF))[2], " pm 0.166"))
